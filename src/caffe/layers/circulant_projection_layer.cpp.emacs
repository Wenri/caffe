...
void CirculantProjectionLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  if (this->param_propagate_down_[0]) {
    ...
    caffe_cpu_fft<Dtype>(M_, K_, top_buffer, diff_buffer); //
    for(int i=0; i<M_; i++)
      for(int j=0; j<K_; j++)
        (data_buffer + i*K_)[(K_-j)%K_] = this->getFlipInput(bottom_data + i*K_, j);//p1
    ...
    caffe_cpu_ifft<Dtype>(M_, K_, conv_buffer, data_buffer);#include <vector>
...
void CirculantProjectionLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
 ...
 this->blobs_[0]->mutable_cpu_diff());
  ...
  if (propagate_down[0]) {
    ...
    for (int i=0; i<K_; i++) weight_buffer[i] = param_buffer[(K_-i)%K_] * (i>0? -1.: 1.);
    FFTmul(param_buffer, top_buffer, data_buffer);//...p8
    */
     //mat version
     for(int i=0; i<K_; i++)
      for(int j=0; j<K_; j++)
